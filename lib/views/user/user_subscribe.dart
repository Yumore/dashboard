...
class _SubscribeTabViewState extends State<SubscribeTabView> with AutomaticKeepAliveClientMixin {
  @override
                  ...
                  child: PopupMenuButton<String>(
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          Text(_selectLetters == 0 ? "字母筛选" : _letters.keys.toList()[_selectLetters]),
                          Icon(
                    ...
                    itemBuilder: (c) => _letters.keys
                        .map((f) => CheckedPopupMenuItem<String>(
                              child: Text(f),
                              value: _letters[f],
                              checked: _selectLetters == _letters.keys.toList().indexOf(f),
                            ))
                        .toList(),
                  ),
                ),
  ...
  Future loadData() async {
    try {
      ...
      var response = await http.get(Uri.parse(Api.userSubscribe(
          widget.type, _subType, Provider.of<AppUserInfo>(context, listen: false).loginInfo.uid, Provider.of<AppUserInfo>(context, listen: false).loginInfo.dmzj_token,
          letter: _letters.values.toList()[_selectLetters], page: _page)));

      List jsonMap = jsonDecode(response.body);

      List<SubscribeItem> detail = jsonMap.map((f) => SubscribeItem.fromJson(f)).toList();
      if (detail != null) {
        setState(() {
          if (_page == 0) {
            _list = detail;
