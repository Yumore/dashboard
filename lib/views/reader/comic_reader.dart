...
import 'package:dashboard/models/comic/comic_web_chapter_detail.dart';
import 'package:dashboard/protobuf/comic/detail_response.pb.dart';
import 'package:dashboard/sql/comic_history.dart';
import 'package:dashboard/views/reader/comic_tc.dart';
import 'package:dashboard/widgets/comic_view.dart';
...
class ComicReaderPage extends StatefulWidget {
  ...
  ComicReaderPage(this.comicId, this.comicTitle, this.chapters, this.item, this.subscribe, {Key? key}) : super(key: key);
...
class _ComicReaderPageState extends State<ComicReaderPage> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      setState(() {
        ...
        _verticalValue = ((_scrollController.offset / _scrollController.position.maxScrollExtent) * 100).toInt().toString() + "%";
      });
    });
  ...
  void setBrightness() async {
    ...
    if (!ConfigHelper.getComicSystemBrightness()) {
      ...
      await ScreenBrightness().setScreenBrightness(ConfigHelper.getComicBrightness());
    }
  ...
  void dispose() {
    ...
    ComicHistoryProvider.getItem(widget.comicId).then((historyItem) async {
      if (historyItem != null) {
        ...
        await ComicHistoryProvider.insert(ComicHistory(widget.comicId, _currentItem!.chapterId, page.toDouble(), 1));
      }
      Utils.changHistory.fire(widget.comicId);
    });

    UserHelper.comicAddComicHistory(widget.comicId, _currentItem!.chapterId, page: page);
    super.dispose();
  }
        ...
        children: <Widget>[
          ...
          Provider.of<AppSetting>(context).comicVerticalMode
              ...
              : Positioned(
                  ...
                  child: InkWell(
                    onTap: () {
                      if (Provider.of<AppSetting>(context, listen: false).comicReadReverse) {
                        previousPage();
                      } else {
                        nextPage();
                      }
          ...
          AnimatedPositioned(
            ...
            child: Container(
              padding: EdgeInsets.only(top: Provider.of<AppSetting>(context).comicReadShowStatusBar ? 0 : MediaQuery.of(context).padding.top),
              width: MediaQuery.of(context).size.width,
              child: Material(
                  color: Color.fromARGB(255, 34, 34, 34),
                  child: ListTile(
                      ...
                      Expanded(
                        child: !_loading
                            ? Provider.of<AppSetting>(context).comicVerticalMode
                                ...
                                : Slider(
                                    value: _selectIndex >= 1 ? _selectIndex.toDouble() : 0,
                    ...
                    children: <Widget>[
                      Provider.of<AppUserInfo>(context).isLogin && widget.subscribe
                          ? createButton(
                              ...
                              Icons.favorite,
                              onTap: () async {
                                if (await UserHelper.comicSubscribe(widget.comicId, cancel: true)) {
                                  setState(() {
                      ...
                      createButton(_detail != null ? "吐槽(${_viewPoints.length})" : "吐槽", Icons.chat_bubble_outline, onTap: openTCPage),
                      ...
                      child: ListView(
                        children: widget.chapters
                            .map((f) => ListTile(
                                  ...
                                  title: Text(
                                    f.chapterTitle,
                                    style: TextStyle(color: f == _currentItem ? Theme.of(context).accentColor : Colors.white),
                                  ),
                                  subtitle: Text(
                                    "更新于" +
                                        TimelineUtil.format(
  ...
  /// 打开吐槽详情页
  void openTCPage() {
    Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => ComicTCPage(_viewPoints, widget.comicId, _currentItem!.chapterId)));
  }
        ...
        child: ComicView.builder(
          ...
          builder: (BuildContext context, int index) {
            if (index > 0 && index <= _detail!.page_url.length) {
              ...
              return PhotoViewGalleryPageOptions.customChild(child: getExtraPage(index));
            }
          },
  ...
  Widget getExtraPage(int index) {
    if (index == 0) {
      return Center(
        child: Text(widget.chapters.indexOf(_currentItem!) == 0 ? "前面没有了" : "上一章", style: TextStyle(color: Colors.grey)),
      );
    }
    ...
    if (index == _detail!.page_url.length + 2) {
      return Center(
        child: Text(widget.chapters.indexOf(_currentItem!) == widget.chapters.length - 1 ? "后面没有了" : "下一章", style: TextStyle(color: Colors.grey)),
      );
    }
    return Center(
      child: Text("出错啦"),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Wrap(
            children: _viewPoints.take(count).map<Widget>((f) => createTucaoItem(f)).toList(),
          ),
          SizedBox(height: 12),
          Center(
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(border: Border.all(color: Colors.grey.withOpacity(0.4)), borderRadius: BorderRadius.circular(16)),
          child: Text(
            item.content,
            style: TextStyle(color: Colors.white),
          ),
              ...
              children: <Widget>[
                ...
                SwitchListTile(
                    ...
                    onChanged: (e) {
                      ...
                      SystemChrome.setEnabledSystemUIOverlays(e ? [] : SystemUiOverlay.values);
                    }),
                SwitchListTile(
                    title: Text(
                      "显示状态信息",
  ...
  Future loadData() async {
    try {
      ...
      if (historyItem != null && historyItem.chapter_id == _currentItem!.chapterId) {
      ...
      await UserHelper.comicAddComicHistory(widget.comicId, _currentItem!.chapterId);
    } catch (e) {
      print(e);
    } finally {
      setState(() {
  ...
  Future loadViewPoint() async {
    try {
      ...
      var response = await http.get(Uri.parse(Api.comicChapterViewPoint(widget.comicId, _currentItem!.chapterId)));

      List jsonMap = jsonDecode(response.body);
      List<ComicChapterViewPoint> ls = jsonMap.map((f) => ComicChapterViewPoint.fromJson(f)).toList();
