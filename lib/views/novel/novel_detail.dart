...
class _NovelDetailPageState extends State<NovelDetailPage> with AutomaticKeepAliveClientMixin {
  @override
                ...
                actions: <Widget>[
                  Provider.of<AppUserInfo>(context).isLogin && _isSubscribe
                      ? IconButton(
                          ...
                          onPressed: () async {
                            var result = await UserHelper.novelSubscribe(widget.novelId, cancel: true);
                            if (result) {
                              setState(() {
                                _isSubscribe = false;
                              });
                      ...
                      : IconButton(
                          ...
                          onPressed: () async {
                            var result = await UserHelper.novelSubscribe(widget.novelId);
                            if (result) {
                              setState(() {
                                _isSubscribe = true;
                              });
                  ...
                  IconButton(icon: Icon(Icons.share), onPressed: () => Share.share("${_detail!.name}\r\nhttp://q.dmzj.com/${widget.novelId}/index.shtml")),
                ],
                bottom: TabBar(tabs: [
                  Tab(text: "详情"),
                  Tab(text: "章节"),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          SizedBox(height: 2),
                          Text(
                            "最后更新:" + DateUtil.formatDate(DateTime.fromMillisecondsSinceEpoch(_detail!.lastUpdateTime.toInt() * 1000), format: "yyyy-MM-dd"),
                            style: TextStyle(color: Colors.grey),
                          ),
                        ],
                      ),
  ...
  Widget cerateVolume() {
    return Scaffold(
      body: volumes.length != 0
          ? ListView.builder(
              itemCount: volumes.length,
              itemBuilder: (ctx, i) {
                var f = volumes[i];
                var his = f.chapters.firstWhere((x) => x.chapterId == historyChapter);
                return Padding(
                  ...
                  child: Container(
                    ...
                    child: ExpansionTile(
                      ...
                      subtitle: his != null ? Text("上次看到:" + his.chapterName) : null,
                      children: f.chapters.map((item) {
                        return InkWell(
                          onTap: () async {
                            await Utils.openNovelReader(context, widget.novelId, volumes, item, novelTitle: _detail!.name, isSubscribe: _isSubscribe);
                            updateHistory();
                          },
                          child: Container(
                            width: double.infinity,
                            ...
                            child: Text(
                              item.chapterName,
                              style: TextStyle(color: item.chapterId == historyChapter ? Theme.of(context).accentColor : Theme.of(context).textTheme.bodyText1!.color),
                            ),
                          ),
                        );
                      }).toList(),
  ...
  void openRead() async {
    ...
    if (historyChapter != 0) {
      ...
      for (var item in volumes) {
        var first = item.chapters.firstWhere((f) => f.chapterId == historyChapter);
        chapterItem = first;
      }
      await Utils.openNovelReader(context, widget.novelId, volumes, chapterItem!, novelTitle: _detail!.name, isSubscribe: _isSubscribe);
      updateHistory();
      return;
    } else {
      await Utils.openNovelReader(context, widget.novelId, volumes, volumes[0].chapters[0], novelTitle: _detail!.name, isSubscribe: _isSubscribe);
      updateHistory();
    }
  }
