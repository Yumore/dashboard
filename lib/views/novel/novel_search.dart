...
class NovelSearchBarDelegate extends SearchDelegate<String> {
  NovelSearchBarDelegate({String searchFieldLabel = "输入关键字搜索轻小说", TextInputType? keyboardType, TextInputAction textInputAction = TextInputAction.search})
  ...
  Widget buildResults(BuildContext context) {
    ...
    return FutureBuilder<List<NovelSearchResultItem>>(
      future: loadData(),
      builder: (BuildContext context, AsyncSnapshot<List<NovelSearchResultItem>> snapshot) {
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            ...
            return ListView(
              children: snapshot.data!.map((f) => createItem(context, f)).toList(),
            );
        }
        // unreachable
      },
  ...
  Widget buildSuggestions(BuildContext context) {
    if (query.length == 0) {
      return FutureBuilder<List<SearchHotWord>>(
        future: loadHotWord(),
        builder: (BuildContext context, AsyncSnapshot<List<SearchHotWord>> snapshot) {
          switch (snapshot.connectionState) {
            case ConnectionState.none:
              return Container();
            case ConnectionState.active:
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Padding(
                        ...
                        child: Wrap(
                          children: snapshot.data!.map((f) => createWorditem(context, f.name, f.id, type: 2)).toList(),
                        ))
                  ],
                ),
              );
  ...
  Widget createWorditem(BuildContext context, String title, int id, {int type = 2}) {
    return Padding(
      padding: EdgeInsets.all(4),
      child: InkWell(
        child: Container(
          padding: EdgeInsets.symmetric(horizontal: 12, vertical: 4),
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: Border.all(color: Theme.of(context).accentColor)),
          child: Text(
            title,
                  ...
                  children: <Widget>[
                    ...
                    Text.rich(
                      TextSpan(children: [
                        ...
                        TextSpan(text: item.authors, style: TextStyle(color: Colors.grey, fontSize: 14))
                      ]),
                    ),
                    SizedBox(
                      height: 2,
  ...
  Future<List<NovelSearchResultItem>> loadData() async {
    try {
      ...
      List<NovelSearchResultItem> detail = ls.map((i) => NovelSearchResultItem.fromJson(i)).toList();
      if (detail != null) {
        return detail;
      }
    } catch (e) {
  ...
  Future<List<SearchHotWord>> loadHotWord() async {
    try {
      ...
      List ls = jsonDecode(response.body);
      List<SearchHotWord> detail = ls.map((i) => SearchHotWord.fromJson(i)).toList();
      if (detail != null) {
        return detail;
      }
    } catch (e) {
