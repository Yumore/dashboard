...
class _ComicCategoryPageState extends State<ComicCategoryPage> with AutomaticKeepAliveClientMixin {
  @override
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, i) => Container(
          child: Card(
            child: InkWell(
              onTap: () {
                Utils.openPage(context, _list[i].tag_id, 11, title: _list[i].title);
              },
              child: Column(
                children: <Widget>[
                  ClipRRect(borderRadius: BorderRadius.circular(4), child: Utils.createCacheImage(_list[i].cover, 200, 200)),
                  SizedBox(height: 4),
                  Flexible(
  ...
  Future loadData() async {
    try {
      ...
      List<ComicCategoryItem> detail = jsonMap.map((i) => ComicCategoryItem.fromJson(i)).toList();
      if (detail != null) {
        setState(() {
          _list = detail;
        });
