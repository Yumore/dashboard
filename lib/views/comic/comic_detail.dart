...
import 'package:dashboard/app/api/comic.dart';
...
import 'package:dashboard/models/comic/comic_related_model.dart';
...
class _ComicDetailPageState extends State<ComicDetailPage> with AutomaticKeepAliveClientMixin {
        ...
        ? DefaultTabController(
            ...
            child: Scaffold(
              appBar: AppBar(
                ...
                bottom: TabBar(tabs: [Tab(text: "详情"), Tab(text: "评论"), Tab(text: "相关")]),
              ),
              body: TabBarView(
                children: [
                  createDetail(),
                            ...
                            style: ButtonStyle(
                              shape: MaterialStateProperty.all(
                                RoundedRectangleBorder(
                                  ...
                                  side: BorderSide(
                                    color: f.data[i].chapterId == historyChapter ? Theme.of(context).accentColor : Colors.grey.withOpacity(0.6),
                                  ),
                                ),
                              ),
                            ),
    ...
    return Padding(
      ...
      child: ButtonTheme(
        ...
        child: OutlinedButton(
          style: ButtonStyle(
            shape: MaterialStateProperty.all(
              RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: BorderSide(color: Theme.of(context).accentColor)),
            ),
          ),
          child: Text(
            text,
  ...
  Widget _getItem(String title, List items,
      {Icon? icon, Function? ontap, bool needSubTitle = true, int count = 3, int type = 1, double ratio = 3 / 5.2, double imgWidth = 270, double imgHeight = 360}) {
    return Offstage(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  GridView.builder(
                    ...
                    itemBuilder: (context, i) =>
                        _getComicItemBuilder(items[i].id, type, items[i].cover, items[i].name, author: needSubTitle ? items[i].status : "", url: '', width: imgWidth, height: imgHeight),
                  ),
                ],
              ),
            ),
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    GridView.builder(
                      ...
                      padding: EdgeInsets.all(2),
                      gridDelegate:
                          SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: MediaQuery.of(context).size.width ~/ 120, mainAxisSpacing: 8.0, crossAxisSpacing: 8.0, childAspectRatio: 6 / 2),
                      itemBuilder: (context, i) {
                        ...
                        return OutlinedButton(
                          style: ButtonStyle(
                            shape: MaterialStateProperty.all(
                              RoundedRectangleBorder(
                                side: BorderSide(color: f.data[i].chapterId == widget.historyChapter ? Theme.of(context).accentColor : Colors.grey.withOpacity(0.4)),
                              ),
                            ),
                          ),
                          child: Text(
  ...
  void openRead() async {
    ...
    if (widget.historyChapter != 0) {
      ...
      for (var item in widget.detail.chapters) {
        var first = item.data.firstWhere((f) => f.chapterId == widget.historyChapter);
        if (first != null) {
          chpters = item;
          _item = first;
        }
