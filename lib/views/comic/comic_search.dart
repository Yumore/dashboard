...
class ComicSearchBarDelegate extends SearchDelegate<String> {
  ...
  Widget buildResults(BuildContext context) {
    ...
    return FutureBuilder<List<ComicNSSearchItem>>(
      future: loadData(),
      builder: (BuildContext context, AsyncSnapshot<List<ComicNSSearchItem>> snapshot) {
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Padding(
                      ...
                      child: Wrap(
                        children: snapshot.data!.map((f) => createWorditem(context, f.name, f.id, type: 1)).toList(),
                      ))
                ],
              ),
            );
  ...
  Widget createWorditem(BuildContext context, String title, int id, {int type = 1}) {
    return Padding(
                  ...
                  children: <Widget>[
                    ...
                    Text.rich(
                      TextSpan(children: [
                        ...
                        TextSpan(text: item.author, style: TextStyle(color: Colors.grey, fontSize: 14))
                      ]),
                    ),
                    SizedBox(
                      height: 2,
                    ),
                    Text(item.status, style: TextStyle(color: Colors.grey, fontSize: 14)),
                    // SizedBox(
                    //   height: 2,
                    // ),
                    // Text(
  ...
  Future<List<ComicNSSearchItem>> loadData() async {
    try {
      ...
      var jsonMap = jsonDecode(response.body.substring(20, response.body.lastIndexOf(';')));
      List ls = jsonMap;
      List<ComicNSSearchItem> detail = ls.map((i) => ComicNSSearchItem.fromJson2(i)).toList();
      if (detail != null) {
        return detail;
      }
    } catch (e) {
