...
DioHttpResponse handleResponse(Response? response, {HttpTransformer? httpTransformer}) {
  ...
  if (_isTokenTimeout(response.statusCode)) {
    return DioHttpResponse.failureFromError(UnauthorisedException(message: "没有权限", code: response.statusCode));
  }
  // 接口调用成功
  if (_isRequestSuccess(response.statusCode)) {
    ...
    // 接口调用失败
    return DioHttpResponse.failure(errorMsg: response.statusMessage, errorCode: response.statusCode);
  }
}

DioHttpResponse handleException(Exception exception) {
