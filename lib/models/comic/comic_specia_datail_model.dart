...
class ComicSpecia {
  ...
  ComicSpecia({
    ...
    int? comment_amount,
  })  : _mobile_header_pic = mobile_header_pic,
        _title = title,
        _page_url = page_url,
        _description = description,
        _comics = comics,
        _comment_amount = comment_amount;

  factory ComicSpecia.fromJson(jsonRes) {
    List<ComicSpeciaItem>? comics = jsonRes['comics'] is List ? [] : null;
    if (comics != null) {
      for (var item in jsonRes['comics']) {
        if (item != null) {
          comics.add(ComicSpeciaItem.fromJson(item));
        }
      }
    }

    return ComicSpecia(
      mobile_header_pic: jsonRes['mobile_header_pic'],
      title: jsonRes['title'],
...
class ComicSpeciaItem {
  ...
  ComicSpeciaItem({
    ...
    String? alias_name,
        ...
        _recommend_brief = recommend_brief,
        _recommend_reason = recommend_reason,
        _id = id,
        _name = name,
        _alias_name = alias_name;

  factory ComicSpeciaItem.fromJson(jsonRes) => ComicSpeciaItem(
        cover: jsonRes['cover'],
