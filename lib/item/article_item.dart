...
class ArticleItem extends StatefulWidget {
  ...
  ArticleItem({required this.articleModel, this.isSearch = false, this.id, this.isFromCollect = false, this.onClickCollect});
...
class _ArticleItemState extends State<ArticleItem> {
  ...
  Widget build(BuildContext context) {
    bool isCollect = (widget.isFromCollect ?? false) || (widget.articleModel.collect ?? false);

    String authorTitle;
    String author;

    if (widget.articleModel.author == null || widget.articleModel.author!.isEmpty) {
      authorTitle = "分享人: ";
    ...
    Row row = Row(
      ...
      children: <Widget>[
        Expanded(child: Text("$authorTitle$author", style: TextStyle(color: AppColors.colorTextAuthor, fontSize: 13))),
        Text(
    ...
    Row title = Row(
      children: <Widget>[
        Expanded(
          child: Text.rich(
            widget.isSearch ?? false ? StringUtils.getTextSpan(widget.articleModel.title, widget.id) : TextSpan(text: widget.articleModel.title),
            softWrap: true,
    ...
    Row chapterName = Row(
      ...
      children: <Widget>[
        Container(
          ...
          decoration: BoxDecoration(color: AppColors.colorCBCBD4, borderRadius: BorderRadius.circular(2)),
          child: Text(
            (widget.isSearch ?? false) ? "" : widget.articleModel.chapterName ?? "",
            softWrap: true,
