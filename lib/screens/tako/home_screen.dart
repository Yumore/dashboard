...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> checkForUpdate() async {
    try {
      final response = await RequestService.create().requestGitHubUpdate(latestRelease);
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return AnimatedBuilder(
                                ...
                                builder: (context, child) {
                                  return Transform(
                                    transform: Matrix4.translationValues(-200 * (1.0 - TakoCurveAnimation(animationController, index, popularList.length).value), 0, 0),
                                    child: child,
                                  );
                                },
                              );
                        ...
                        SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              ...
                              return AnimatedBuilder(
                                animation: animationController,
                                child: RecentlyAddedAnimeCard(anime: recentlyAdded[index]),
                                builder: (context, child) {
                                  return Transform(
                                    transform: Matrix4.translationValues(-200 * (1.0 - TakoCurveAnimation(animationController, index, recentlyAdded.length).value), 0, 0),
                                    child: child,
                                  );
                                },
                              );
