...
class SearchListPageState extends State<SearchListPage> {
  ...
  Widget build(BuildContext context) {
    if (listData.isEmpty) {
      ...
      Widget listView = ListView.builder(
        ...
        itemBuilder: (context, i) => ArticleItem(articleModel: listData[i], id: widget.id, isSearch: true),
  ...
  void _articleQuery() {
    ApiManager.instance.queryArticleList(curPage, widget.id, successCallback: (articleListModel) {
      setState(() {
        if (curPage == 0) {
          listData.clear();
        }
