...
class _HotPageState extends State<HotPage> {
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: <Widget>[
        Padding(padding: EdgeInsets.all(8.0), child: Text('大家都在搜', style: TextStyle(color: Theme.of(context).primaryColor, fontSize: 16.0))),
        Wrap(
          spacing: 5.0,
          runSpacing: 1.0,
          children: hotKeys
        ...
        Padding(padding: EdgeInsets.all(8.0), child: Text('常用网站', style: TextStyle(color: Theme.of(context).primaryColor, fontSize: 16.0))),
        Wrap(
          spacing: 5.0,
          runSpacing: 1.0,
          children: friends
              .map((friendModel) => HotKeyCell(
                    ...
                    onPressed: () {
                      Navigator.of(context).push(MaterialPageRoute(builder: (context) {
                        return ArticleDetailPage(title: friendModel.name ?? "", url: friendModel.link ?? "");
                      }));
                    },
                  ))
              .toList(),
...
class HotKeyCell extends StatelessWidget {
  ...
  const HotKeyCell({Key? key, required this.text, required this.onPressed}) : super(key: key);
