...
class ComicModel {
  ...
  static Future<ComicHomeListEntity> getHomeData(BuildContext context) async {
    ...
    var responseJson = await Request.getByHttpClient("http://kanmanapi-main.321mh.com/v1/book/getBookByType?platformname=iphone&booktype=" + bookType.toString() + "&productname=aym&pagesize=20");
  ...
  static Future<List<ComicRecommendEntity>> getRecommentById(String cId) async {
    ...
    (responseJsonRecom as List).forEach((v) {
      comicChapter.add(new ComicRecommendEntity.fromJson(v));
    });
    return comicChapter;
  }
  ...
  static Future<ComicRankEntity> getRankList(String rank_type, String stat_type) async {
    String url = "http://getconfig-globalapi.yyhao.com/app_api/v5/getranklistdetail/?platformname=iphone&productname=aym&rank_type=" + rank_type + "&stat_type=" + stat_type + "&limit=100";
    //获取推荐漫画
    var responseJson = await Request.getByHttpClient(url);
    print(url);
