 ...
 * 漫画书架
 */
class ComicBookRackScene extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => ComicBookRackState();
}

class ComicBookRackState extends State<ComicBookRackScene> with AutomaticKeepAliveClientMixin {
  List<ComicBlock> comicBlocks;
  ...
  Future<void> fetchData() async {
    try {
      //获取关联漫画
      comicBlocks = await ComicDao.getAllCollect();
      setState(() {
        isDataReady = true;
      });
    } catch (e) {
  ...
  List<Widget> getListView() {
    ...
    var addChildrens = comicBlocks.map((comicItem) => ComicBlockItemView(comicItem, TYColor.white)).toList();
