...
class ComicDetailState extends State<ComicDetailScene> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      //对章节进行反序转换
      if (comicDetail != null && comicDetail.comicChapter != null && comicDetail.comicChapter.length > 0) {
        List<ComicDetailInfoComicChapter> chapters = [];
        for (var i = 0; i < comicDetail.comicChapter.length; i++) {
          chapters.add(comicDetail.comicChapter[comicDetail.comicChapter.length - i - 1]);
        }
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: RaisedButton(
                ...
                onPressed: () => AppNavigator.pushComicReader(context, comicDetail, 0),
                child: Text(
                  "开始阅读",
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: TYColor.white, decoration: TextDecoration.none),
                )),
            margin: EdgeInsets.only(right: 15),
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(3), color: TYColor.primary),
          )
        ],
      ),
      height: 60,
  ...
  _collectComic() async {
    ...
    if (_hasCollected) {
      ...
      ComicDao.addCollecte(comicDetail.comicName, widget.url, comicDetail.coverList[0]).then((result) {
        setState(() {
          _hasCollected = true;
        });
      });
          ...
          Container(
              ...
              child: Column(
                children: <Widget>[
                  ...
                  Container(width: width, height: 0.7, color: Color(0xffe1e1e1)),
                  _getBottomReader(),
                ],
              )),
        ]);
