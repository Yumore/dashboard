...
class ComicDetailRecommend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (comicDetail == null || CollectionsUtils.isEmpty(comicDetail)) {
      return Container();
    }
    //推荐的不要太多了，限制为9个
    ...
    var children = blocks.map((comicItem) => ComicBlockItemView(comicItem, TYColor.white)).toList();
    ...
    return SingleChildScrollView(
        child: Column(
          children: <Widget>[
            Container(
              child: Text("骚年们都在看", style: TextStyle(fontSize: 15, color: TYColor.gray, fontWeight: FontWeight.w300, decoration: TextDecoration.none)),
