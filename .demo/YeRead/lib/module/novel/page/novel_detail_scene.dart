...
class NovelDetailSceneState extends State<NovelDetailScene> {
  ...
  _collectComic() async {
    ...
    if (_hasCollected) {
      ...
      NovelDao.addCollecte(novelDetail.title, widget.novelId, novelDetail.cover).then((result) {
        setState(() {
          _hasCollected = true;
        });
      });
    ...
    return Stack(
      children: <Widget>[
        Opacity(
          opacity: navAlpha,
          child: Container(
            decoration: BoxDecoration(color: TYColor.white, boxShadow: Styles.borderShadow),
  ...
  Widget buildComment() {
    if (novelComments == null || novelComments.reviews == null || novelComments.reviews.length == 0) {
      return Container();
    }
    return Container(
      color: Colors.white,
      child: Column(
        ...
        children: <Widget>[
          Container(
            ...
            child: Row(
              children: <Widget>[
                ...
                Text('  写书评', style: TextStyle(fontSize: 14, color: TYColor.primary)),
                SizedBox(width: 15),
              ],
            ),
          ),
          Divider(height: 1),
          Column(
            children: novelComments.reviews.map((comment) => NovelCommentCell(comment)).toList(),
          ),
          Divider(height: 1),
          GestureDetector(
            onTap: () {
  ...
  Widget buildPosts() {
    if (novelPost == null || novelPost.posts == null || novelPost.posts.length == 0) {
      return Container();
    }
    return Container(
      color: Colors.white,
      child: Column(
        ...
        children: <Widget>[
          ...
          Divider(height: 1),
          Column(
            children: novelPost.posts.map((comment) => NovelPostCell(comment)).toList(),
          ),
          Divider(height: 1),
          GestureDetector(
            onTap: () {
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: <Widget>[
                      ...
                      SizedBox(height: 10),
                      NovelSummaryView(novelDetail.longIntro, isSummaryUnfold, changeSummaryMaxLines),
                      NovelDetailCell(
                        iconName: 'img/detail_latest.png',
                        title: '最新',
                        subtitle: novelDetail.lastChapter,
                        updateTime: AppUtils.showNovelTime(novelDetail.updated),
                        attachedWidget: Text(novelDetail.isSerial ? "连载" : "完结", style: TextStyle(fontSize: 14, color: Colors.blue)),
                        onTap: () {
