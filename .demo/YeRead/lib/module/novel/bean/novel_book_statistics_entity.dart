class NovelBookStatisticsEntity {
  ...
  NovelBookStatisticsEntity.fromJson(Map<String, dynamic> json) {
    if (json['female'] != null) {
      female = new List<NovelBookStatistics>();
      (json['female'] as List).forEach((v) {
        female.add(new NovelBookStatistics.fromJson(v));
      });
    }
    if (json['press'] != null) {
      press = new List<NovelBookStatistics>();
      (json['press'] as List).forEach((v) {
        press.add(new NovelBookStatistics.fromJson(v));
      });
    }
    ok = json['ok'];
    if (json['male'] != null) {
      male = new List<NovelBookStatistics>();
      (json['male'] as List).forEach((v) {
        male.add(new NovelBookStatistics.fromJson(v));
      });
    }
    if (json['picture'] != null) {
      picture = new List<NovelBookStatistics>();
      (json['picture'] as List).forEach((v) {
        picture.add(new NovelBookStatistics.fromJson(v));
      });
    }
  }
