...
class VideoModel {
  ...
  static Future<VideoHomeListEntity> getHome() async {
    ...
    VideoHomeListEntity entity = new VideoHomeListEntity(recommends: recommends, updates: updates);
    return entity;
  }
  ...
  static List<Video> getItems(String match) {
    RegExp reg = new RegExp(r'<li class="anime[\s\S]*?href="(.*?)"[\s\S]*?src="(.*?)"[\s\S]*?alt="(.*?)"[\s\S]*?title="(.*?)"[\s\S]*?/li>');
    ...
    for (Match m in matches) {
      ...
      if (cover != "") {
        Video video = new Video(title: title, cover: cover, sId: sid, shortIntro: desc);
        list.add(video);
      }
    }
    return list;
  ...
  static Future<VideoDetailPage> getVideoDetail(String vid) async {
    ...
    List<VideoDetailChapterEntity> chapters = getVideoDetailChapter(responseBody);
  ...
  static VideoInfoEntity getVideoDetailInfo(String responseBody) {
    ...
    info = new VideoInfoEntity(
        time: time, tags: tags, title: title, type: type, originalTitle: originalTitle, company: company, writer: writer, status: status, region: region, cover: cover, describe: describe);
    return info;
  }

  static List<VideoDetailChapterEntity> getVideoDetailChapter(String responseBody) {
    List<VideoDetailChapterEntity> comicChapter = new List<VideoDetailChapterEntity>();
    try {
  ...
  static List<Video> getDetailRecommend(String body) {
    ...
    RegExp reg = new RegExp(r'<li class="anime[\s\S]*?href="(.*?)"[\s\S]*?data-src="(.*?)"[\s\S]*?anime_icon1_name">(.*?)</div');
  ...
  static List<VideoDetailComment> getDetailComment(String body) {
    ...
    RegExp reg = new RegExp(r'<li class="comment[\s\S]*?commentcell_user">(.*?)<[\s\S]*?commentcell_content"> <div>(.*?)<[\s\S]*?ime asciifont">(.*?)<');
    Iterable<Match> matches = reg.allMatches(recomBody);
  ...
  static List<VideoCatalogItem> getCatalogListItems(String body) {
    RegExp reg = new RegExp(r'href="(.*?)" >([\s\S]*?)</a');
  ...
  static List<VideoInfoEntity> getVideoCataLogItems(String responseBody) {
    RegExp regAll = new RegExp(
        r'class="cell blockdiff[\s\S]*?href="/detail/(.*?)"[\s\S]*?src="(.*?)"[\s\S]*?alt="(.*?)"[\s\S]*?class="newname">(.*?)<[\s\S]*?cell_imform_name">(.*?)<[\s\S]*?cell_imform_desc">([\s\S]*?)</div>');
    Iterable<Match> matchesAll = regAll.allMatches(responseBody);
    String infoMatch = "";
    print(matchesAll);
