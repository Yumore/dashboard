 ...
 * 动画书架
 */
class VideoCollectScene extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => VideoCollectState();
}

class VideoCollectState extends State<VideoCollectScene> with AutomaticKeepAliveClientMixin {
  List<Video> comicBlocks = [];
  ...
  Future<void> fetchData() async {
    try {
      //获取关联漫画
      comicBlocks = await VideoDao.getAllCollect();
      setState(() {
        isDataReady = true;
      });
    } catch (e) {
  ...
  List<Widget> getListView() {
    ...
    var addChildrens = comicBlocks.map((comicItem) => VideoBlockItemView(comicItem, TYColor.white)).toList();
