...
class LookHistory {
  ...
  LookHistory({@required this.name, @required this.targetRouter});
}

class SearchHistoryList {
  static SpUtil _sp;
  static SearchHistoryList _instance;
  static List<LookHistory> _searchHistoryList = [];

  static SearchHistoryList _getInstance(SpUtil sp) {
    if (_instance == null) {
      _sp = sp;
      String json = sp.get("searchHistory");
...
//  List<SearchHistory> _searchHistoryList = [];
  ...
  SearchHistoryList.fromJSON(String jsonData) {
    ...
    jsonList.forEach((value) {
      _searchHistoryList.add(LookHistory(name: value['name'], targetRouter: value['targetRouter']));
    });
  }

  List<LookHistory> getList() {
    return _searchHistoryList;
  }

  clear() {
    _sp.remove("searchHistory");
    _searchHistoryList = [];
  }

  save() {
    _sp.putString("searchHistory", this.toJson());
  }

  add(LookHistory item) {
    print("_searchHistoryList> ${_searchHistoryList.length}");
    for (LookHistory value in _searchHistoryList) {
  ...
  toJson() {
    ...
    _searchHistoryList.forEach((LookHistory value) {
