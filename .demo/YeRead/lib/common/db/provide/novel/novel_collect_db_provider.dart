 ...
 * Date: 2018-08-07
 */
class NovelCollectDbProvider extends BaseDbProvider {
  final String name = 'NovelCollect';
  ...
  NovelCollectDbProvider.fromMap(Map map) {
    id = map[columnId];
  ...
  Future _getProvider(Database db, String sid) async {
    List<Map<String, dynamic>> maps = await db.query(name, columns: [columnId, columnTitle, columnSid, columnCover], where: "$columnSid = ?", whereArgs: [sid]);
    if (maps.length > 0) {
      NovelCollectDbProvider provider = NovelCollectDbProvider.fromMap(maps.first);
      return provider;
    }
    return null;
  }
  ...
  ///获取事件数据
  Future<List<NovelBook>> getAll() async {
    Database db = await getDataBase();
    List<NovelBook> list = new List();
    List<Map<String, dynamic>> maps = await db.query(name);
    if (maps.length > 0) {
      maps.forEach((map) {
        NovelBook comicBlock = new NovelBook();
        NovelCollectDbProvider provider = NovelCollectDbProvider.fromMap(map);
        comicBlock.title = provider.title;
