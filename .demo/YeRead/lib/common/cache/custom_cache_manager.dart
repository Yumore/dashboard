...
abstract class BaseCacheManager {
  ...
  BaseCacheManager(this._cacheKey, {Duration maxAgeCacheObject = const Duration(days: 30), int maxNrOfCacheObjects = 200, FileFetcher fileFetcher}) {
    _fileBasePath = getFilePath();

    _maxAgeCacheObject = maxAgeCacheObject;
    _maxNrOfCacheObjects = maxNrOfCacheObjects;
    store = new CacheStore(_fileBasePath, _cacheKey, _maxNrOfCacheObjects, _maxAgeCacheObject);
  ...
  Future<FileInfo> downloadFile(String url, {Map<String, String> authHeaders, bool force = false}) async {
    return await webHelper.downloadFile(url, authHeaders: authHeaders, ignoreMemCache: force);
  }
  ...
  Future<File> putFile(String url, Uint8List fileBytes, {String eTag, Duration maxAge = const Duration(days: 30), String fileExtension = "file"}) async {
  ...
  Future<File> putStringFile(String url, String fileBytes, {String eTag, Duration maxAge = const Duration(days: 30), String fileExtension = "file"}) async {
    var cacheObject = await store.retrieveCacheData(url);
    if (cacheObject == null) {
      var relativePath = "${new Uuid().v1()}.$fileExtension";
      cacheObject = new CacheObject(url, relativePath: relativePath);
