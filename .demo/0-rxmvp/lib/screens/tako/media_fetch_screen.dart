...
class _MediaFetchScreenState extends State<MediaFetchScreen> {
  ...
  Future<void> fetchVideoFile() async {
    if (webViewManagerController.isWebView) {
      var mediaUrl = await AnimeService().fetchIframeEmbedded(animeUrl).catchError((_) {
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.done) {
                        return SizedBox.fromSize(
                          size: Size(MediaQuery.of(context).size.width / 1.5, MediaQuery.of(context).size.height / 1.5),
                          child: TakoPlayWebView(
                            ...
                            onLoadingFinished: (_webViewController) async {
                              ...
                              // Ads Block
                              for (var i = 0; i < 8; i++) {
                                await _webViewController.runJavascriptReturningResult("document.getElementsByTagName('iframe')[$i].style.display='none';");
                                await _webViewController.runJavascriptReturningResult("document.getElementsByClassName('jw-icon jw-icon-display jw-button-color jw-reset')[0].click();");
                              }
                              // Fetching VidStreaming Url
                              String rawUrl = await _webViewController.runJavascriptReturningResult("document.getElementsByClassName('jw-video jw-reset')[0].attributes.src.value;");
                              for (var i = 0; i < 8; i++) {
                                rawUrl = await _webViewController.runJavascriptReturningResult("document.getElementsByClassName('jw-video jw-reset')[0].attributes.src.value;");
                              }
                              if (rawUrl == 'null' || !rawUrl.contains(RegExp(r'(.)[0-9]+(p.mp4)'))) {
                                hasError.value = true;
                              } else {
                                ...
                                String resolutionCount = await _webViewController.runJavascriptReturningResult(
                                ...
                                for (var j = 0; j < double.parse(resolutionCount) - 1; j++) {
                                  String rawResolution = await _webViewController.runJavascriptReturningResult(
                                  ...
                                  String resolution = rawResolution.replaceFirst('"', ' ').trim().replaceFirst('"', ' ').trim();
                                  String quality = resolution.split(' P')[0];
                                  _qualityList.add(quality);
                                  resolutions.putIfAbsent(resolution, () => url.replaceFirst(RegExp(r'(.)[0-9]+(p.mp4)'), '.${quality}p.mp4'));
                                }
                                for (var qlt in _qualityList) {
                                  if (mediaFetchController.defaultQuality.contains(qlt)) {
                                    _filteredUrl = url.replaceAll(RegExp(r'(.)[0-9]+(p.mp4)'), '.${qlt}p.mp4');
                                  }
                                }
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Obx(
                  () => Image.asset(
                    hasError.value ? 'assets/gif/anime-cry.gif' : 'assets/gif/anime${1 + _random.nextInt(25)}.gif',
                    width: 350,
                    height: 200,
                  ),
                ),
                  ...
                  () => Visibility(
                    ...
                    child: MaterialButton(
                      ...
                      child: Text(
                        ...
                        style: TakoTheme.darkTextTheme.headline3!.copyWith(fontSize: 17.0),
                      ),
                    ),
                  ),
                ),
