...
class _SearchScreenState extends State<SearchScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    controller = AnimationController(duration: Duration(milliseconds: takoAnimationDuration), vsync: this);
        ...
        body: Obx(
          () => hasValue.value
              ? FutureBuilder<AnimeResults>(
                  future: AnimeService().getAnimes(provider.requestSearchResponse(title.value)),
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          return AnimatedBuilder(
                            ...
                            builder: (BuildContext context, Widget? child) {
                              return Transform(
                                transform: Matrix4.translationValues(0, 100 * (1.0 - TakoCurveAnimation(controller, index, list.length).value), 0),
                                child: child,
                              );
                            },
                          );
  ...
  Future<void> _saveToRecentSearches(searchText) async {
    if (searchText != null) {
      ...
      Set<String> allSearches = pref.getStringList('takoRecentSearches')?.toSet() ?? {};
  ...
  Future<void> _deleteSearch(searchText) async {
    final pref = await SharedPreferences.getInstance();
    final newList = pref.getStringList('takoRecentSearches')!.where((result) => result != searchText).toList();
