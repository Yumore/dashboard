...
class DioAdapter extends PinkNetAdapter {
  ...
  Future<PinkNetResponse<T>> send<T>(PinkBaseRequest request) async {
    ...
    try {
      if (request.httpMethod() == HttpMethod.GET) {
        ...
        if (request.file != null) {
          response = await Dio().post(request.url(), options: options, data: request.file, onSendProgress: sendProgress);
        } else {
    ...
    if (error != null) {
      ///抛出PinkNetError
      throw PinkNetError(response?.statusCode ?? -1, error.toString(), data: await buildRes(response, request));
    }
    return buildRes(response, request);
  }

  /// 构建PinkNetResponse
  Future<PinkNetResponse<T>> buildRes<T>(Response? response, PinkBaseRequest request) {
    return Future.value(
      PinkNetResponse(
        data: response?.data,
        request: request,
