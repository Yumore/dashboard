...
class LoginDao {
  ...
  static registration(String userName, String email, String validateCode, String password, String rePassword) {
    return _send(email, password, rePassword: rePassword, validateCode: validateCode, userName: userName);
  }

  static forgetPwd(String email, String validateCode, String password, String rePassword) async {
    ForgetPwdRequest request = ForgetPwdRequest();
    request.add("email", email).add("validate_code", validateCode).add("password", password).add("re_password", rePassword);
    var result = await PinkNet.getInstance().fire(request);
    return result;
  }

  static _send(String email, String password, {rePassword, validateCode, userName}) async {
    BaseRequest request;
    if (rePassword != null || validateCode != null) {
      request = RegistrationRequest();
      request.add("email", email).add("validate_code", validateCode).add("username", userName).add("password", password).add("re_password", rePassword);
    } else {
