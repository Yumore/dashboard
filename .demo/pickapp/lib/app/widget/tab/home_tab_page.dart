...
class HomeTabPage extends StatefulWidget {
  ...
  const HomeTabPage({Key? key, this.categoryId = 0, this.bannerList}) : super(key: key);
...
class _HomeTabPageState extends PinkBaseTabState<HomeMo, PostMo, HomeTabPage> {
      ...
      ? ListView(
          ...
          children: [
            ...
            if (bannerList != null && bannerList!.length > 0)
              Container(
                margin: EdgeInsets.only(bottom: setHeight(10), top: setHeight(10), left: setWidth(10), right: setWidth(10)),
                child: BannerCard(
                  bannerList,
            ...
            Container(
              padding: EdgeInsets.only(top: setHeight(24), left: setWidth(16), right: setWidth(16)),
              child: GridView.builder(
                  shrinkWrap: true,
                  physics: NeverScrollableScrollPhysics(),
                  itemCount: dataList.length,
                  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 510 / 534),
                  itemBuilder: (BuildContext context, int index) {
                    return dataList[index].postType == "video" || dataList[index].postType == "collection"
            ...
            dataList.isNotEmpty && dataList.length >= 20 ? LoadingMore() : Container()
          ],
        )
      : ListView(
          children: [NotFound()],
  ...
  Future<HomeMo> getData(int pageIndex) async {
    HomeMo result = await HomeDao.get(widget.categoryId, page: pageIndex, size: 20, sort: default_sort);
    return result;
  }
  ...
  headerSort() {
    return Container(
      decoration: BoxDecoration(color: Colors.white, border: Border(top: BorderSide(width: 1, color: Color.fromRGBO(247, 247, 247, 1)))),
      padding: EdgeInsets.only(top: setHeight(25), bottom: setHeight(25), left: setWidth(25), right: setWidth(25)),
      child: Row(
          ...
          return ClipRRect(
            ...
            child: Material(
              child: InkWell(
                ...
                child: Container(
                  padding: EdgeInsets.only(left: setWidth(30), right: setWidth(30), top: setHeight(10), bottom: setHeight(10)),
                  child: Text(
                    "${e["name"]}",
                    style: TextStyle(fontSize: setSp(36), color: default_sort == e["key"] ? Colors.white : Color.fromRGBO(160, 160, 160, 1)),
                  ),
                ),
              ),
