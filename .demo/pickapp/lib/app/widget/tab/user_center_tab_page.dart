...
class _UserCenterTabPageState extends PinkBaseTabState<DynamicMo, PostMo, UserCenterTabPage> {
  @override
      ...
      ? widget.slug == "dynamic"
          ? Container(
              child: ListView.builder(
                  itemCount: dataList.length, itemBuilder: (BuildContext context, int index) => Container(margin: EdgeInsets.only(bottom: 10), child: DynamicLargeCard(contentModel: dataList[index]))),
            )
          : ListView(
  ...
  Future<DynamicMo> getData(int page) async {
    DynamicMo result = await UserPostDao.get(widget.userId, widget.slug, page, 20);
    return result;
  }
