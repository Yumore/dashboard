...
class AwaitWatchTabPage extends StatefulWidget {
  ...
  const AwaitWatchTabPage({Key? key, this.refresh = true, required this.postType}) : super(key: key);

  @override
  _AwaitWatchTabPageState createState() => _AwaitWatchTabPageState();
}

class _AwaitWatchTabPageState extends PinkBaseTabState<DynamicMo, PostMo, AwaitWatchTabPage> {
  late var awaitWatchPostIds;

  @override
  void initState() {
    awaitWatchPostIds = widget.postType == "await_watch" ? GetStorage().read("awaitWatchPost") : GetStorage().read("historyWatchPost");
    super.initState();
  }
      ...
      ? ListView(
          ...
          children: [
            GridView.builder(
              ...
              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 1, childAspectRatio: setHeight(1080) / setWidth(250)),
  ...
  Future<DynamicMo> getData(int page) async {
    DynamicMo result = await AwaitWatchDao.get(awaitWatchPostIds != null ? jsonEncode(awaitWatchPostIds) : "", page: page, size: 10);
    return result;
  }
