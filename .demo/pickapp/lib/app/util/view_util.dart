...
/// 带缓存的image
Widget cachedImage(String url, {double width = 200, double height = 150, String img = 'assets/icon/default.png'}) {
  return CachedNetworkImage(
      ...
      imageUrl: url.startsWith('https://') || url.startsWith('http://') ? url + "@250w_160h.webp" : PinkConstants.ossDomain + '/' + url);
}

/// 黑色线性渐变
blackLinearGradient({bool fromTop = false}) {
  return LinearGradient(begin: fromTop ? Alignment.topCenter : Alignment.bottomCenter, end: fromTop ? Alignment.bottomCenter : Alignment.topCenter, colors: [
    Colors.black54,
    Colors.black45,
    Colors.black38,
    Colors.black26,
...
borderLine(BuildContext context, {bottom: true, top: false}) {
  BorderSide borderSide = BorderSide(width: 0.5, color: Color.fromRGBO(235, 236, 238, 1));
  return Border(bottom: bottom ? borderSide : BorderSide.none, top: top ? borderSide : BorderSide.none);
}
...
/// 竖线
longString({double width = 0.5, double height = 20, Color color = Colors.grey}) {
  return SizedBox(
    width: width,
    height: height,
    child: DecoratedBox(
...
Widget iconShare(String name, String path) {
  return Container(
    margin: EdgeInsets.only(left: setWidth(25), right: setWidth(25), top: setHeight(30)),
    child: Column(
      children: [
        Image.asset(
          path,
        ...
        Text(
          ...
          style: TextStyle(decoration: TextDecoration.none, fontSize: setSp(30), color: Color.fromRGBO(151, 158, 164, 1), fontWeight: FontWeight.w400),
        ),
      ],
    ),
  );
