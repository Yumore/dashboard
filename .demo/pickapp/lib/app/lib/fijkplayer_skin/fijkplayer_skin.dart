...
class _CustomFijkPanelState extends State<CustomFijkPanel> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  ...
  Future<void> changeCurPlayVideo(int tabIdx, int activeIdx) async {
    ...
    await player.reset().then((_) {
      String curTabActiveUrl = _videoSourceTabs.video![tabIdx]!.list![activeIdx]!.url!;
          ...
          child: Align(
            ...
            child: Padding(
              padding: EdgeInsets.only(
                left: 20,
                top: showConfig.stateAuto && !player.value.fullScreen ? barGap : 0,
              ),
              child: IconButton(
                iconSize: 30,
                onPressed: () {
              ...
              tabs: _videoSourceTabs.video!
                  ...
                  .map((int tabIdx) => Tab(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(width: setWidth(3), color: Color.fromRGBO(255, 255, 255, .2)),
                          borderRadius: BorderRadius.all(Radius.circular(setRadius(20))),
                        ),
  ...
  List<Widget> _createTabConList() {
    ...
    _videoSourceTabs.video!.asMap().keys.forEach((int tabIdx) {
      List<Widget> playListBtns = _videoSourceTabs.video![tabIdx]!.list!.asMap().keys.map((int activeIdx) {
        return Container(
          height: setHeight(100),
          width: setWidth(400),
          margin: EdgeInsets.only(bottom: setHeight(20)),
          padding: EdgeInsets.only(left: setWidth(40), right: setWidth(40), top: 0, bottom: 0),
          child: ElevatedButton(
            style: ButtonStyle(
              shape: MaterialStateProperty.all(
                RoundedRectangleBorder(
                  side: BorderSide(width: setWidth(3), color: tabIdx == widget.curTabIdx && activeIdx == widget.curActiveIdx ? primary : Color.fromRGBO(255, 255, 255, .2)),
                  borderRadius: BorderRadius.circular(setRadius(20)),
                ),
              ),
              elevation: MaterialStateProperty.all(0),
              backgroundColor: MaterialStateProperty.all(Color.fromRGBO(0, 0, 0, .4)),
            ),
            onPressed: () {
            ...
            child: Text(
              ...
              overflow: TextOverflow.ellipsis,
              style: TextStyle(
                color: tabIdx == widget.curTabIdx && activeIdx == widget.curActiveIdx ? primary : Colors.white,
              ),
            ),
          ),
        );
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            ...
            child: Center(
              child: Padding(
                padding: EdgeInsets.only(top: showConfig.stateAuto && !widget.player.value.fullScreen ? barGap : 0),
                child: slot,
              ),
            ),
          ),
  ...
  Widget _buildErrorStateSlotWidget() {
    return Center(
      child: Column(
        ...
        children: [
          SizedBox(
            height: showConfig.stateAuto && !widget.player.value.fullScreen ? barGap : 0,
          ),
          // 失败图标
          Icon(
            Icons.error,
  ...
  Widget build(BuildContext context) {
    ...
    if (_playerState == FijkState.error) {
    ...
    } else if ((_playerState == FijkState.asyncPreparing || _playerState == FijkState.initialized) && !_isPlaying) {
      ws.add(
        _buildPublicFrameWidget(
      ...
      if (_lockStuff == true && showConfig.lockBtn && widget.player.value.fullScreen) {
        ws.add(
...
class _buildGestureDetectorState extends State<_buildGestureDetector> {
  ...
  void _playerValueChanged() async {
    ...
    bool isOverFlow = widget.curActiveIdx + 1 >= _videoSourceTabs.video![widget.curTabIdx]!.list!.length;
  ...
  Widget _buildBottomBar(BuildContext context) {
    ...
    double currentValue = _seekPos > 0 ? _seekPos : (_isHorizontalMove ? _dargPos.inMilliseconds.toDouble() : _currentPos.inMilliseconds.toDouble());
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            ...
            left: 0,
            right: 0,
            child: showConfig.bottomPro && _hideStuff && _duration.inMilliseconds != 0
                ? Container(
                    alignment: Alignment.bottomLeft,
                    height: setHeight(6),
        ...
        child: Column(
          children: <Widget>[
            ...
            showConfig.topBar
                ? _buildTopBar()
                : Container(
                    height: showConfig.stateAuto && !widget.player.value.fullScreen ? barFillingHeight : barHeight,
                  ),
            // 中间按钮
            Expanded(
              child: Stack(
