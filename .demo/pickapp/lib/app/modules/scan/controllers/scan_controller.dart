  ...
  Future<UserMeta?> getUserMeta(String qrContent) async {
    ...
    if (res["type"] == "user_center") {
      ...
      try {
        ...
        Get.back();
        result.postInfo!.postType == "video" ? Get.toNamed("/user-center", arguments: {"profileMo": result.postInfo!}) : Get.toNamed("/user-center", arguments: {"profileMo": result.postInfo!});
      } on NeedLogin catch (e) {
        showWarnToast(e.message);
      } on NeedAuth catch (e) {
        showWarnToast(e.message);
