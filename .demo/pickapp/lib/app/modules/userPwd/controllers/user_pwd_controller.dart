...
class UserPwdController extends GetxController {
  ...
  void checkInput() {
    bool enable;
    if (isNotEmpty(oldPassword) && isNotEmpty(validateCode) && isNotEmpty(newPassword) && isNotEmpty(reNewPassword)) {
      enable = true;
    } else {
      enable = false;
    }
  ...
  void send() async {
    try {
      var result = await UserUpdateDao.updatePassword(oldPassword, profileMo.value.email, validateCode, newPassword, reNewPassword);
      if (result['code'] == 1000) {
        showToast("修改成功");
        Get.back();
      } else {
  ...
  void sendForgetPwdEmail() async {
    try {
      if (isEmail(profileMo.value.email)) {
        var result = await ChangePwdEmailValidateCodeDao.get(profileMo.value.email);
        if (result['code'] == 1000) {
          showToast("验证码发送成功");
        } else {
          showWarnToast(result['msg']);
