...
class ForgetPwdController extends GetxController {
  ...
  void checkInput() {
    bool enable;
    if (isNotEmpty(email.value) && isNotEmpty(validateCode) && isNotEmpty(password) && isNotEmpty(rePassword)) {
      enable = true;
    } else {
      enable = false;
    }
  ...
  void send() async {
    try {
      var result = await LoginDao.forgetPwd(email.value, validateCode, password, rePassword);
      if (result['code'] == 1000) {
        showToast("修改成功");
