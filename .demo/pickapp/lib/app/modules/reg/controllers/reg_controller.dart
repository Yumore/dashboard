...
class RegController extends GetxController {
  ...
  void checkInput() {
    bool enable;
    if (isNotEmpty(userName) && isNotEmpty(password) && isNotEmpty(rePassword)) {
      enable = true;
    } else {
      enable = false;
    }
  ...
  void send() async {
    try {
      var result = await LoginDao.registration(userName, email.value, validateCode, password, rePassword);
      if (result['code'] == 1000) {
        showToast("注册成功");
