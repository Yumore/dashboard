        ...
        child: Row(
          children: [
            ...
            InkWell(
                onTap: () async {
                  try {
                    var result = await UserUpdateDao.update("username", controller.profileMo.value.username);
                    if (result["code"] == 1000) {
                      showToast("修改成功");
                      Get.back();
                    } else {
  ...
  _content() {
    return UserTextarea(
        controller: TextEditingController.fromValue(TextEditingValue(
            ...
            selection: TextSelection.fromPosition(TextPosition(affinity: TextAffinity.downstream, offset: controller.profileMo.value.username.length)))),
        onChanged: (value) {
