...
class ChatController extends GetxController with SingleGetTickerProviderMixin {
  ...
  void sendMsg(String message, {String pic = "", int media = 1}) {
    ...
    _websocket.send(message, pic, currentUserMeta.userId, sendUserMeta.sendId, media);
  ...
  void _loadData() async {
    try {
      loading = true;
      ChatList result = await ChatDao.get(sendUserMeta.sendUserMeta!.userId, page: page, size: 10);
  ...
  void onInit() {
    ...
    var num = GetStorage().read("contact_send_id_${sendUserMeta.sendUserMeta!.userId}") != null ? GetStorage().read("contact_send_id_${sendUserMeta.sendUserMeta!.userId}") : 0;
    var numAll = GetStorage().read("contact_send_id_all") != null ? GetStorage().read("contact_send_id_all") : 0;
    GetStorage().write("contact_send_id_all", numAll - num);
    GetStorage().write("contact_send_id_${sendUserMeta.sendUserMeta!.userId}", 0);
    //先在启动时初始化
