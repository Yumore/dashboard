...
class ChatView extends GetView<ChatController> {
                      ...
                      children: [
                        controller.dataList.length > 0
                            ...
                            : Container(
                                child: Text(
                                  "发个消息聊聊呗!",
                                  style: TextStyle(color: Colors.grey, fontSize: setSp(40)),
                                ),
  ...
  _chatAvatar(bool isSelf) {
    return Container(
      ...
      ], border: Border.all(width: 1, color: Colors.grey), borderRadius: BorderRadius.circular(35)),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(35),
        child: InkWell(
          onTap: () {
            Get.toNamed("/user-center", arguments: {
              "profileMo": isSelf ? controller.currentUserMeta : controller.sendUserMeta.sendUserMeta,
            });
          },
          child: cachedImage(isSelf ? controller.currentUserMeta.avatar : controller.sendUserMeta.sendUserMeta!.avatar, width: 35, height: 35),
        ),
      ),
    );
  }
    ...
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
            ...
            color: value.userId == controller.currentUserMeta.userId ? Color.fromRGBO(235, 249, 255, 1) : Colors.white,
            borderRadius: BorderRadius.only(
              topLeft: Radius.circular(value.userId == controller.currentUserMeta.userId ? 20 : 0),
              topRight: Radius.circular(value.userId == controller.currentUserMeta.userId ? 0 : 20),
            ...
            children: [
              ...
              if (value.media == 4)
                Container(
                  child: InkWell(
                      onTap: () {
                        Get.toNamed("/photo-dia", parameters: {"image": PinkConstants.ossDomain + "/" + value.pic});
                      },
                      child: ConstrainedBox(
                        constraints: BoxConstraints(minWidth: setWidth(400), minHeight: setHeight(100)
                            // minWidth: double.infinity, // //宽度尽可能大
                            ),
                        child: Image.network(
      ...
      child: Column(
        children: controller.dataList.map((value) {
          return Container(
            ...
            child: Row(
              children: [
                value.userId == controller.currentUserMeta.userId ? Container() : _chatAvatar(value.userId == controller.currentUserMeta.userId),
                Expanded(
                    child: Row(
                  mainAxisAlignment: value.userId == controller.currentUserMeta.userId ? MainAxisAlignment.end : MainAxisAlignment.start,
                  children: [
                    Column(
                      mainAxisAlignment: value.userId == controller.currentUserMeta.userId ? MainAxisAlignment.end : MainAxisAlignment.start,
                      crossAxisAlignment: value.userId == controller.currentUserMeta.userId ? CrossAxisAlignment.end : CrossAxisAlignment.start,
                      children: [
                        Container(
                          margin: EdgeInsets.only(left: 10, right: 10, top: 10),
                          child: Text(
                            value.userId == controller.currentUserMeta.userId ? "${controller.currentUserMeta.username}" : "${controller.sendUserMeta.sendUserMeta!.username}",
                ...
                value.userId == controller.currentUserMeta.userId ? _chatAvatar(value.userId == controller.currentUserMeta.userId) : Container(),
              ],
            ),
          );
        }).toList(),
      ...
      child: Row(
        children: [
          appBarButton("assets/icon/pic.png", () async {
            List<AssetEntity>? fileList = await getImagePicker(context, maxAssets: 9);
          ...
          Expanded(
            ...
            child: ClipRRect(
              ...
              child: Stack(
                children: [
                  Container(
                    padding: EdgeInsets.only(left: setWidth(20), right: setWidth(90)),
                    height: 32,
                    alignment: Alignment.center,
                    child: Obx(() => TextField(
                          onTap: () {
                          ...
                          controller: TextEditingController.fromValue(TextEditingValue(
                              ...
                              selection: TextSelection.fromPosition(TextPosition(affinity: TextAffinity.downstream, offset: controller.msg.value.length)))),
