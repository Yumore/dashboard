...
class VideoController extends GetxController with SingleGetTickerProviderMixin {
  ...
  videoFormat() async {
    ...
    await player.value.setOption(FijkOption.formatCategory, "headers", "referer:${contentModel.value.video.video[0].list[0].url}");
    // 格式化json转对象
    videoSourceTabs.value = VideoSourceFormat.fromJson(contentModel.value.video.toJson());
  }
  ...
  Future<void> send(String value) async {
    try {
      if (value.isNotEmpty) {
        ...
        var result = await CommentDao.post(contentModel.value.postId, value, "post");
        if (result["code"] == 1000) {
  ...
  void loadDetail() async {
    try {
      VideoDetailMo result = await VideoDetailDao.get(contentModel.value.postId.toString());
  ...
  void onReady() {
    ...
    // 埋点日志（点击事件）
    LogDao.post({"action": "click", "userId": contentModel.value.userMeta!.userId.toString(), "postId": contentModel.value.postId.toString(), "algorithmCombine": "C2"},
  ...
  void onClose() {
    // 埋点日志（阅读时间）
    if (readTime >= 10) {
      LogDao.post({"action": "read", "userId": contentModel.value.userMeta!.userId.toString(), "postId": contentModel.value.postId.toString(), "algorithmCombine": "C2"},
          readTime: readTime.toString(), categoryId: contentModel.value.categoryId);
    }
    player.value.stop();
