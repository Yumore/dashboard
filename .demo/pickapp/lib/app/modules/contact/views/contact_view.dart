...
class ContactView extends GetView<ContactController> {
        ...
        child: Row(
          children: [
              ...
              Get.defaultDialog(
                  ...
                  confirm: GetxDialogButton(
                    ...
                    onTap: () async {
                      ...
                      var result = await ContactDao.post(controller.uidController.text);
                      if (result["code"] == 1000) {
                        controller.loadData();
                        showToast("添加成功!");
                      } else {
            ...
            Expanded(
              ...
              child: Text(
                "消息",
                style: TextStyle(fontSize: setSp(42), fontWeight: FontWeight.w600),
              ),
            )),
            Container(
  ...
  Widget _headerMenu() {
    return Row(
      children: [
        headerTextButton("回复我的", "assets/icon/msg.png", Color.fromRGBO(62, 211, 159, 1)),
        headerTextButton("@我", "assets/icon/aite.png", Colors.yellow),
        headerTextButton("收到的赞", "assets/icon/re_star.png", Colors.red),
        headerTextButton("系统通知", "assets/icon/noti.png", Color.fromRGBO(66, 200, 229, 1)),
      ],
    );
  }
  ...
  List<Widget> _content(context) {
    return [
      Container(
        width: Get.width,
        color: Colors.white,
        padding: EdgeInsets.only(top: setHeight(15), left: setWidth(25), bottom: setHeight(15)),
        child: Text(
          "聊天列表",
      ...
      Expanded(
        child: MediaQuery.removePadding(removeTop: true, context: context, child: Obx(() => RefreshIndicator(color: primary, child: _contactLists(), onRefresh: controller.loadData))),
      )
    ];
  }

  Widget _contactLists() {
    return controller.contactList.value.list != null && controller.contactList.value.list!.isNotEmpty
        ? Container(
            color: Colors.white,
            child: ListView(
  ...
  Widget _getNoReadMsg(int sendId) {
    return GetStorage().read("contact_send_id_$sendId") != null && GetStorage().read("contact_send_id_$sendId") != 0
        ? Container(
            child: Text(
              GetStorage().read("contact_send_id_$sendId") != null && GetStorage().read("contact_send_id_$sendId") != 0 ? "${GetStorage().read("contact_send_id_$sendId")}" : "",
  ...
  Widget _contactItem(contactMeta) {
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () {
          Get.toNamed("/chat", arguments: {"currentUserMeta": controller.userMeta, "sendUserMeta": contactMeta});
        },
        child: Column(
          children: [
            Container(
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(borderRadius: BorderRadius.circular(setRadius(130)), border: Border.all(width: 1, color: Colors.grey)),
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(setRadius(130)),
                      child: cachedImage(contactMeta.sendUserMeta!.avatar, width: setR(130), height: setR(130)),
                    ),
                  ),
                  Expanded(
                      child: Row(
                    ...
                    children: [
                        ...
                        children: [
                          Container(
                            child: Text(
                              ...
                              style: TextStyle(fontSize: setSp(40), color: primary, fontWeight: FontWeight.w600),
                            ),
                          ),
                          hiSpace(height: setHeight(15)),
                          Container(
                            child: contactMeta.chatMo!.media == 1
                                ? ExpressionText(contactMeta.chatMo!.content, TextStyle(color: Colors.black))
                                : contactMeta.chatMo!.media == 0 && contactMeta.chatMo!.userId == 0 && contactMeta.chatMo!.sendId == 0
                                    ? Text(
                                        "暂无消息",
                                        style: TextStyle(fontSize: setSp(35), color: Colors.grey, fontWeight: FontWeight.w400),
                                      )
                                    : Text(
                                        "[图片]",
                                        style: TextStyle(fontSize: setSp(35), color: Colors.grey, fontWeight: FontWeight.w400),
                      ...
                      Column(
                        children: [
                          Text(
                            formatDate(DateTime.parse(contactMeta.updateTime.substring(0, 19))),
