...
class ProfilePostView extends GetView<ProfilePostController> {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
                child: Container(
              margin: EdgeInsets.only(right: controller.postType == "await_watch" || controller.postType == "history_watch" ? setWidth(0) : setWidth(60)),
              alignment: Alignment.center,
              child: Text(controller.title()),
            )),
            controller.postType == "await_watch" || controller.postType == "history_watch"
                ? Container(
                    margin: EdgeInsets.only(right: setWidth(30)),
                    child: appBarButton("assets/icon/del.png", () {
                      controller.postType == "await_watch" ? GetStorage().write("awaitWatchPost", []) : GetStorage().write("historyWatchPost", []);
                      showToast("删除成功");
                    }),
                  )
                : Container(),
  ...
  _content() {
    return Expanded(
      child: controller.postType == "await_watch" || controller.postType == "history_watch"
          ? AwaitWatchTabPage(
