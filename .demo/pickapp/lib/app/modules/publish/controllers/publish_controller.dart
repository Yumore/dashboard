...
class PublishController extends GetxController {
  ...
  void publish() async {
    ...
    if (type == "post" && (cover.value.isEmpty || title.value.isEmpty || postHtml!.isEmpty)) {
      showWarnToast("请填写完整内容");
      return;
    }
    if (type == "video" && (cover.value.isEmpty || title.value.isEmpty || collection.value.video[0].list[0].url.isEmpty)) {
      showWarnToast("请填写完整内容");
      return;
    }
    if (type == "collection" && (cover.value.isEmpty || title.value.isEmpty || collection.value.video[0].list[0].url.isEmpty || collection.value.video[0].list[0].name.isEmpty)) {
      showWarnToast("请填写完整内容");
      return;
    }
    ...
    var result = await PublishPostDao.get(title.value, contents.value, cover.value, categoryId.value, type, jsonEncode(collection.value.toJson()));
    if (result["code"] == 1000) {
      showToast("文章发布成功");
